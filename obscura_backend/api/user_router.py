...
from application.use_cases.users.change_user_info import ChangeUserInfoUseCase
...
from api.exceptions.excpetions import (
    HTTPUserAlreadyExists,
    InternalServerError,
    HTTPDuplicatePassword,
    HTTPWrongPassword,
    HTTPInvalidRecoveryKey,
)
...
router = APIRouter(prefix="/users", tags=["User Domain"])


@router.post("/create-new-account", status_code=201)
def create_new_account(
    user_name: str, first_name: str, last_name: str, password: str, db: db_dependency
):
    try:
        ...
        create_account_use_case = CreateAccountUseCase(
            repo=user_repo, key_generator=key_generator
        )
...
def change_user_password(
    old_password: str, new_password: str, user: user_dependency, db: db_dependency
):
    repo = UserRepoImpl(session=db)
    use_case = ChangePassUseCase(repo=repo)
    try:
        use_case.execute(
            username=user["sub"], old_password=old_password, new_password=new_password
        )
...
def reset_user_password(
    username: str, recovery_key: str, new_password: str, db: db_dependency
):
...
@router.put("/update-user-info", status_code=201)
def update_user_info(
    f_name: str, l_name: str, db: db_dependency, user: user_dependency
):
    repo = UserRepoImpl(db)
    use_case = ChangeUserInfoUseCase(repo)
    use_case.execute(user["sub"], f_name, l_name)
    return {"message": "User info has been updated successfully."}
